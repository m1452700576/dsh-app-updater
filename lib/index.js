...
// DSH 核心包的实际安装位置（桌面应用内置）
const CORE_PKG_DIRS = [
  '/Applications/DeepSeek Harness.app/Contents/Resources/app/node_modules/@deepseek-ai',
  '/Applications/DeepSeek Harness.app/Contents/Resources/runtime/node_modules/@deepseek-ai',
  '/Applications/DeepSeek Harness.app/Contents/Resources/app/node_modules/.pnpm/node_modules/@deepseek-ai',
]
// npm 上要检查的核心包
const CORE_PACKAGES = [
  { name: '@deepseek-ai/dsh', label: 'DSH 核心 (dsh)' },
  { name: '@deepseek-ai/dsh-session', label: '会话 (dsh-session)' },
  { name: '@deepseek-ai/dsh-agent', label: 'Agent (dsh-agent)' },
]
...
/** 读取本地安装的 DSH 核心包版本（从桌面应用内置 node_modules）。 */
function readCoreVersion(pkgName) {
  const short = pkgName.replace('@deepseek-ai/', '')
  for (const base of CORE_PKG_DIRS) {
    try {
      const pkg = JSON.parse(readFileSync(base + '/' + short + '/package.json', 'utf8'))
      if (typeof pkg.version === 'string') return pkg.version.trim()
    } catch { /* try next dir */ }
  }
  return undefined
...
/** 读取 npm dist-tags（latest/next）对应的版本号。 */
async function fetchNpmDistTags(pkgName, timeoutMs = 10_000) {
  ...
  try {
    ...
    return {
      ...
      next: typeof tags.next === 'string' ? tags.next : undefined,
      alpha: typeof tags.alpha === 'string' ? tags.alpha : undefined,
    }
  } finally {
    clearTimeout(timer)
  }
...
/** 探测 npm 上 DSH 核心包是否有比本地更新的版本。 */
async function probeNpmCore() {
  const results = []
  for (const pkg of CORE_PACKAGES) {
    const local = readCoreVersion(pkg.name)
    let tags
    try {
      tags = await fetchNpmDistTags(pkg.name)
    } catch (error) {
      results.push({ pkg: pkg.name, label: pkg.label, local, latest: null, next: null, outdated: false, error: error instanceof Error ? error.message : String(error) })
      continue
    }
    const candidates = [tags.latest, tags.next].filter(v => v !== undefined)
    let target = null
    for (const v of candidates) {
      if (target === null || compareVersions(v, target) > 0) target = v
    }
    const outdated = local !== undefined && target !== null && compareVersions(target, local) > 0
    results.push({
      pkg: pkg.name,
      label: pkg.label,
      local,
...
async function checkStatus(config) {
  ...
  // 额外检查 npm 上的 DSH 核心包是否有更新
  const coreUpdates = await probeNpmCore().catch(() => [])
  const coreOutdated = coreUpdates.filter(u => u.outdated)
  if (found) {
    return { ok: true, current, cli, latest: found.latest, outdated: true, downloadUrl: found.asset.browser_download_url, assetName: found.asset.name, notes: found.notes, source: 'github:' + found.repo, sourceType: 'github', repo: found.repo, htmlUrl: found.htmlUrl, repos, errors, coreUpdates }
  }
  if (coreOutdated.length > 0) {
    // 桌面壳无更新，但核心包有更新 → 提示用户
    const newest = coreOutdated.map(u => ({ pkg: u.pkg, label: u.label, local: u.local, target: u.target }))
    // 尝试找到当前桌面壳对应的官方 release 页面
    const desktopRepo = 'agent-earth/deepseek-harness-desktop'
    const officialUrl = 'https://github.com/' + desktopRepo + '/releases'
    return {
      ok: true, current, cli, latest: null, outdated: true,
      downloadUrl: null, assetName: null,
      notes: '检测到 DSH 核心包有更新（npm），当前桌面应用尚未打包该版本。核心包由桌面应用内置，需要等待官方发布新版桌面应用（或使用 dsh CLI 的 next 版本）。',
      source: 'npm:' + coreOutdated[0].pkg, sourceType: 'npm',
      officialUrl,
      coreUpdates, repos, errors, coreOutdated: newest,
    }
  }
  return { ok: true, current, cli, latest: null, outdated: false, downloadUrl: null, notes: null, source: 'github', sourceType: 'github', repos, errors, coreUpdates }
}
...
/** 判断下载内容是否为 HTML 错误页（镜像代理失败时的常见响应）。 */
function looksLikeHtml(buffer) {
  if (!buffer || buffer.length < 16) return false
  const head = buffer.subarray(0, 1024).toString('utf8').toLowerCase()
  if (/^\s*<!doctype html|^\s*<html/i.test(head)) return true
  if (/^\s*(404|not found|error|bad gateway|access denied)\b/i.test(head) && head.length < 2000) return true
  return false
}

/** 校验文件 magic bytes 是否符合声明后缀的安装包类型。 */
function hasInstallerMagic(buffer, fileName) {
  const name = String(fileName || '').toLowerCase()
  if (name.endsWith('.dmg')) {
    // DMG 尾部 512 字节内应有 "koly" 标记
    const tail = buffer.subarray(Math.max(0, buffer.length - 512))
    return tail.includes(Buffer.from('koly'))
  }
  if (name.endsWith('.zip')) return buffer.subarray(0, 2).toString('hex') === '504b' // PK
  if (name.endsWith('.pkg')) return buffer.subarray(0, 4).toString('latin1') === 'xar!' // xar archive
  if (name.endsWith('.exe')) return buffer.subarray(0, 2).toString('latin1') === 'MZ'
  if (name.endsWith('.appimage')) return true
  return true // 未知类型不校验，交给系统判断
}
...
async function downloadAndOpen(config, url, name) {
  // 优先直连，避免镜像返回错误页；镜像作为后备
  const source = await pickSource(config, url)
  let response = await fetch(source, { headers: { 'user-agent': UA } })
  let buffer = null
  let downloadedFrom = source
  let downloadedName = name
  if (!response.ok) throw new Error('download failed: HTTP ' + response.status + (response.status === 403 ? '（GitHub 拒绝访问或限流）' : ''))
  // 先读取一次，判断内容是否有效；若镜像返回 HTML 错误页，回退到直连 GitHub
  {
    const total0 = Number(response.headers.get('content-length')) || 0
    if (total0 > MAX_DOWNLOAD_BYTES) throw new Error('文件过大（>1 GiB），已拒绝下载')
    const reader0 = typeof response.body !== 'undefined' && response.body !== null ? response.body.getReader() : undefined
    const chunks0 = []
    let done0 = 0
    activeDownload = { phase: 'downloading', done: 0, total: total0, fileName: name ?? null }
    if (reader0 !== undefined) {
      for (;;) {
        const step = await reader0.read()
        if (step.done) break
        chunks0.push(step.value)
        done0 += step.value.length
        if (done0 > MAX_DOWNLOAD_BYTES) throw new Error('文件过大（>1 GiB），已中止下载')
        activeDownload.done = done0
      }
    } else {
    ...
    buffer = Buffer.concat(chunks0)

    // 若返回的是 HTML 错误页（镜像代理失败），且当前源是镜像，则回退到直连 GitHub
    if (looksLikeHtml(buffer)) {
      if (source !== url) {
        activeDownload = { phase: 'downloading', done: 0, total: 0, fileName: name ?? null }
        response = await fetch(url, { headers: { 'user-agent': UA } })
        if (!response.ok) throw new Error('镜像返回了错误页且直连失败: HTTP ' + response.status)
        const total2 = Number(response.headers.get('content-length')) || 0
        const reader2 = typeof response.body !== 'undefined' && response.body !== null ? response.body.getReader() : undefined
        const chunks2 = []
        let done2 = 0
        activeDownload = { phase: 'downloading', done: 0, total: total2, fileName: name ?? null }
        if (reader2 !== undefined) {
          for (;;) {
            const step = await reader2.read()
            if (step.done) break
            chunks2.push(step.value)
            done2 += step.value.length
            if (done2 > MAX_DOWNLOAD_BYTES) throw new Error('文件过大（>1 GiB），已中止下载')
            activeDownload.done = done2
          }
        } else {
          const whole2 = Buffer.from(await response.arrayBuffer())
          chunks2.push(whole2)
          done2 = whole2.length
          activeDownload.done = done2
        }
        ...
        downloadedFrom = url
        if (looksLikeHtml(buffer)) throw new Error('下载内容不是安装包（可能是网络错误页），请稍后重试')
      } else {
        throw new Error('下载内容不是安装包（可能是网络错误页），请稍后重试')
      }
    }

    // 校验文件 magic bytes，防止下载到损坏/错误文件
    if (downloadedName && !hasInstallerMagic(buffer, downloadedName)) {
      throw new Error('下载的安装包校验失败（文件可能损坏），已停止打开')
    }
  }
  const sha256 = sha256Hex(buffer)
  let checksumVerified = null
  let fileName = downloadedName
  ...
  if (expected) {
    if (expected.hash === sha256) { checksumVerified = true }
      ...
      activeDownload = { phase: 'failed', done: buffer.length, total: buffer.length, fileName, error: 'checksum-mismatch' }
      throw new Error('校验和不匹配，已拒绝下载：预期 ' + expected.hash + '，实际 ' + sha256)
    }
  }
  activeDownload = { phase: 'saving', done: buffer.length, total: buffer.length, fileName }
  writeFileSync(file, buffer)
  addQuarantine(file, downloadedFrom)
  addMotw(file, downloadedFrom)
  let opened = false
  await new Promise((resolve) => {
    const child = openInstaller(file)
    child.on('error', () => resolve())
    child.on('close', () => { opened = true; resolve() })
  })
  activeDownload = { phase: 'done', done: buffer.length, total: buffer.length, fileName, file, opened }
  return { file, bytes: buffer.length, opened, sha256, checksumVerified }
}
/* ---------- plugin ---------- */
