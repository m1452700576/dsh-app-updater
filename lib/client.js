...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function AppUpdateEntry(props) {
      var wide = props.wide;
      var state = React.useState({ open: false, minimized: false, view: { kind: "idle" }, available: false, progress: null, customUrl: "", urlCheck: null, confirming: false });
      ...
      React.useEffect(function() {
        var ignored = window.localStorage.getItem('dsh-updater-ignored-version');
      ...
      var probe = React.useCallback(function () {
        fetch("/api/dsh-update/status").then(function (r) { return r.json(); }).then(function (data) {
          set(function (s) { return { open: s.open, minimized: s.minimized, view: s.view, available: !!data.outdated, progress: s.progress, customUrl: s.customUrl, urlCheck: s.urlCheck, confirming: s.confirming }; });
        }).catch(function () {});
      }, []);
      React.useEffect(function () { probe(); }, [probe]);

      function openPanel() {
        set({ open: true, minimized: false, view: { kind: "checking" }, available: available, progress: progress, customUrl: "", urlCheck: null, confirming: false });
        fetch("/api/dsh-update/status").then(function (r) { return r.json(); }).then(function (data) {
          set(function (s) { return { open: true, minimized: false, view: data.error ? { kind: "error", message: data.error } : { kind: "result", data: data }, available: !!data.outdated, progress: s.progress, customUrl: "", urlCheck: null, confirming: false }; });
        }).catch(function (e) {
          set(function (s) { return { open: true, minimized: false, view: { kind: "error", message: String(e) }, available: s.available, progress: s.progress, customUrl: "", urlCheck: null, confirming: false }; });
        });
      }

      function minimize() { set(function (s) { return { open: false, minimized: true, view: s.view, available: s.available, progress: s.progress, customUrl: s.customUrl, urlCheck: s.urlCheck, confirming: false }; }); }
      function restore() { set(function (s) { return { open: true, minimized: false, view: s.view, available: s.available, progress: s.progress, customUrl: s.customUrl, urlCheck: s.urlCheck, confirming: s.confirming }; }); }
      ...
      function download() {
        set({ open: true, minimized: false, view: { kind: "downloading" }, available: available, progress: { phase: "downloading", done: 0, total: 0, fileName: null }, customUrl: customUrl, urlCheck: null, confirming: false });
      ...
      if (open) {
        ...
        if (view.kind === "downloading") {
          ...
          body = [
            ...
            d.notes ? h("div", { key: "notes", style: styles.notes }, String(d.notes).slice(0, 2000)) : null,
            ...
            (d.outdated || (urlCheck && urlCheck.reachable && !urlCheck.privateHost))
              ? h("div", { key: "dl", style: styles.actions }, 
                  ...
                  d.outdated ? h("button", { style: Object.assign({}, styles.ghost, { marginLeft: 8 }), onClick: function() { 
                    window.localStorage.setItem('dsh-updater-ignored-version', d.latest);
        ...
        if (confirming && (view.kind === "result")) {
          ...
          panel = h("div", { style: styles.overlay, onClick: function (e) { if (e.target === e.currentTarget) set({ open: false, minimized: false, view: view, available: available, progress: progress, customUrl: customUrl, urlCheck: urlCheck, confirming: false }); } },
            h("div", { style: styles.card },
              ...
              h("div", { style: styles.actions },
                ...
                h("button", { style: styles.ghost, onClick: function () { set({ open: false, minimized: false, view: view, available: available, progress: progress, customUrl: customUrl, urlCheck: urlCheck, confirming: false }); } }, L.close)
              )
            )
          );
        }
